...
class MasterKeyProvider(object):
    ...
    def master_keys_for_encryption(self, encryption_context, plaintext_rostream, plaintext_length=None):
        ...
        .. note::
            ...
            only those Master Keys added to this instance of this Provider using the add_master_key
            function or discovered by decrypting EncryptedDataKeys.
    ...
    def add_master_key(self, key_id):
        ...
        if key_id not in self._key_index:
            ...
            self._key_index[key_id] = master_key
    ...
    def master_key(self, key_id):
        """Returns a Master Key based on the specified key_id, adding it to this provider if not already present.
            ...
            if member.provider_id == encrypted_data_key.key_provider.provider_id:
                ...
                if isinstance(member, MasterKey):
                    ...
                    try:
                        ...
                        master_key = member.master_key(encrypted_data_key.key_provider.key_info)
                ...
                except (IncorrectMasterKeyError, DecryptKeyError) as e:
                    _LOGGER.debug(
...
class MasterKey(MasterKeyProvider):
    ...
    def __hash__(self):
        """Returns value needed for Python to uniquely identify this object.

        :returns: Hash identifier
        :rtype: int
        """
        digest = hashes.Hash(hashes.SHA256(), backend=default_backend())
        digest.update(six.b(str(self.key_provider)))
        return cryptography.utils.int_from_bytes(digest.finalize(), byteorder='big')
    ...
    def __eq__(self, other):
        ...
        :rtype: bool
        """
        return type(self) == type(other) and self.key_id == other.key_id
    ...
    def encrypt_data_key(self, data_key, algorithm, encryption_context):
        ...
        return self._encrypt_data_key(data_key, algorithm, encryption_context)
    ...
    def decrypt_data_key(self, encrypted_data_key, algorithm, encryption_context):
        ...
        decrypted_data_key = self._decrypt_data_key(encrypted_data_key, algorithm, encryption_context)
        aws_encryption_sdk.internal.utils.source_data_key_length_check(
