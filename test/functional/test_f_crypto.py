...
@pytest.mark.parametrize(
    "algorithm",
    [
        aws_encryption_sdk.AlgorithmSuite.AES_128_GCM_IV12_TAG16_HKDF_SHA256_ECDSA_P256
        for i in range(10)
    ]
    + [
        aws_encryption_sdk.AlgorithmSuite.AES_256_GCM_IV12_TAG16_HKDF_SHA384_ECDSA_P384
        for i in range(10)
    ],
)
def test_ecc_static_length_signature(algorithm):
    ...
    signature = _ecc_static_length_signature(
        key=private_key, algorithm=algorithm, digest=digest
    )
...
def test_signer_key_bytes_cycle():
    ...
    signer = Signer(
        algorithm=aws_encryption_sdk.AlgorithmSuite.AES_256_GCM_IV12_TAG16_HKDF_SHA384_ECDSA_P384,
        key=key,
    )
    ...
    assert (
        new_signer.key.private_numbers().private_value
        == signer.key.private_numbers().private_value
    )
