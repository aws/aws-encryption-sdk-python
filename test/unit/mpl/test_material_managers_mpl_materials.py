...
try:
    ...
    # Ensure references to these mocks exist, even if they aren't used in a non-MPL context
    mock_mpl_cmm = None
    mock_mpl_encryption_materials = None
    mock_mpl_decrypt_materials = None
...
def test_GIVEN_test_has_mpl_is_False_THEN_cmm_has_mpl_is_False():
    ...
    assert hasattr(aws_encryption_sdk.materials_managers.mpl.materials, "_HAS_MPL")
    assert aws_encryption_sdk.materials_managers.mpl.materials._HAS_MPL is False
...
def test_GIVEN_test_has_mpl_is_True_THEN_cmm_has_mpl_is_True():
    ...
    assert hasattr(aws_encryption_sdk.materials_managers.mpl.materials, "_HAS_MPL")
    assert aws_encryption_sdk.materials_managers.mpl.materials._HAS_MPL is True


@pytest.mark.skipif(HAS_MPL, reason="Test should only be executed without MPL in installation")
def test_GIVEN_test_has_mpl_is_False_WHEN_create_MPLCMMHandler_THEN_raise_ImportError():
    with pytest.raises(ImportError):
        MPLEncryptionMaterials(mpl_materials="doesn't matter")
