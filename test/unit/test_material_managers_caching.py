...
@pytest.mark.parametrize('too_old, too_many_messages, too_many_bytes, result', (
    (True, True, True, True),
...
def test_cache_entry_has_exceeded_limits(mocker, too_old, too_many_messages, too_many_bytes, result):
    ...
    mocker.patch.object(
        ...
        '_cache_entry_has_encrypted_too_many_bytes',
        return_value=too_many_bytes
    )
    mocker.patch.object(
        CachingCryptoMaterialsManager,
        '_cache_entry_has_encrypted_too_many_messages',
        return_value=too_many_messages
    )
    ccmm = build_ccmm()
...
@pytest.mark.parametrize('plaintext_length, algorithm_provided, algorithm_safe_to_cache, response', (
...
def test_get_encryption_materials_cache_hit_expired_entry(
    ...
    patch_build_encryption_materials_cache_key.assert_called_once_with(
        ...
        request=patch_encryption_materials_request.return_value
    )
