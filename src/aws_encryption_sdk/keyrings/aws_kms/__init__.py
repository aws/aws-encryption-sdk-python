...
class KmsKeyring(Keyring):
    ...
    Set ``additional_key_ids`` to specify additional CMKs that the keyring will use to encrypt the data key.

    The keyring will attempt to use any CMKs
    identified by CMK ARN in either ``generator_key_id`` or ``additional_key_ids`` on decrypt.
    ...
    :param List[str] additional_key_ids: Key IDs that will be used to encrypt and decrypt data keys (optional)
    ...
    def __attrs_post_init__(self):
        ...
        key_ids_provided = self._generator_key_id is not None or self._additional_key_ids
        ...
        child_keyrings = [
            ...
            for key_id in self._additional_key_ids
        ]
