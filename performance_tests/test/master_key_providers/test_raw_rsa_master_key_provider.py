...
@click.option('--output_file',
              default='/'.join(__file__.split("/")[:-3]) + '/results/raw_rsa_key_provider_create')
def create(
    n_iters: int,
    output_file: str
):
...
@click.option('--n_iters',
              default=PerfTestUtils.DEFAULT_N_ITERS)
@click.option('--output_file',
              default='/'.join(__file__.split("/")[:-3]) + '/results/raw_rsa_key_provider_encrypt')
def encrypt(
...
@click.option('--n_iters',
              default=PerfTestUtils.DEFAULT_N_ITERS)
@click.option('--output_file',
              default='/'.join(__file__.split("/")[:-3]) + '/results/raw_rsa_key_provider_decrypt')
def decrypt(
